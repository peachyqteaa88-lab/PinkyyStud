/* ============================================================
   PINKYSTUDY - APLIKASI JUALAN ONLINE PINK PASTEL
...
const DB = {
  get(key, def = null) {
    const v = localStorage.getItem('pinkystudy_' + key);
    return v ? JSON.parse(v) : def;
  },
  set(key, val) {
    localStorage.setItem('pinkystudy_' + key, JSON.stringify(val));
  }
};
...
function seedData() {
  ...
  if (!DB.get('products')) {
    DB.set('products', [
      {
        id: 'p1', name: 'Bolpoin', price: 3000, unit: '1 biji',
        step: 1, minQty: 1,
        desc: 'Bolpoin tinta hitam halus, nyaman untuk mencatat. Cocok untuk tugas sekolah & kuliah. Harga per biji, bisa beli berapa saja.',
        emoji: '🖊️', stock: 25, category: 'Alat Tulis', active: true, isService: false
      },
      {
        id: 'p2', name: 'Margin Biologi', price: 13000, unit: '1 biji',
        step: 1, minQty: 1,
      ...
      {
        id: 'p3', name: 'Kertas Fisika', price: 3000, unit: '10 lembar',
        step: 10, minQty: 10,
        desc: 'Kertas folio bergaris untuk catatan fisika. Dijual per 10 lembar. Bisa pilih paket 10, 20, 30, 40 lembar.',
        emoji: '📄', stock: 50, category: 'Alat Tulis', active: true, isService: false
      },
      {
        id: 'p4', name: 'Jasa Pembuatan Daftar Pustaka', price: 7000, unit: '10 daftar pustaka',
        step: 10, minQty: 10,
        desc: 'Jasa pembuatan daftar pustaka rapi & sesuai format. Upload file kamu, kami kerjakan. Hasil dikirim via email. Maksimal pemesanan H-1.',
...
function getCart() {
  ...
  Object.keys(cart).forEach(k => {
    if (k.startsWith(prefix)) mine[k] = cart[k];
  });
  return mine;
}
...
function cartTotalItems() {
  const mine = getCart();
  return Object.values(mine).reduce((a, b) => a + b, 0);
}
...
function render() {
  ...
  switch (currentPage) {
    ...
    case 'admin': html = renderAdmin(optsAdminTab || 'dashboard'); break;
  ...
  renderFooter();
  // attach rating stars
  if (currentPage === 'rating') attachStars();
}
let optsAdminTab = 'dashboard';
...
function productCardHTML(p) {
  ...
  if (admin) {
    actions = `
      ...
      <button class="btn btn-outline btn-sm" onclick="editProductForm('${p.id}')">✏️ Edit</button>
    `;
  } else {
    actions = `
  ...
  return `
    <div class="section-head">
      <div>
        ...
        <p class="sub">${prods.length} produk tersedia${admin ? ' — Mode Admin: kamu dapat mengelola produk di sini' : ''}</p>
      </div>
...
function renderDetail(id) {
  ...
  if (!p) return `<div class="empty-state"><h3>Produk tidak ditemukan</h3><button class="btn btn-primary" onclick="navigate('shop')">Kembali ke Jualan</button></div>`;
...
function changeDetailQty(id, delta) {
  ...
  render(); // refresh tombol disabled
}
  ...
  return `
    ...
    <div class="admin-panel">
      ...
      ${items.map(x => `
        <div class="pay-info-row">
          <span>${x.p.emoji} ${x.p.name} ×${x.qty}</span>
          <b>${rupiah(x.sub)}</b>
        </div>
      `).join('')}
      <div class="summary-total"><span>Total</span><span>${rupiah(total)}</span></div>
    ...
    ${hasService ? `
      <div class="admin-panel" style="border-color:var(--blue-pastel);">
        ...
        <div class="field-file mb-16" onclick="document.getElementById('co-file').click()">
          <input type="file" id="co-file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onchange="onFilePick(this)">
          <span class="icon">📎</span>
          <b id="file-label">Upload file yang ingin dibuatkan daftar pustakanya</b>
          <p class="muted mt-16" style="font-size:.8rem;">Format: PDF, DOC, DOCX, JPG, PNG</p>
        </div>
        <div class="form-grid">
          <div class="field">
            <label>Permintaan Selesai — Tanggal</label>
            <input type="date" id="co-deadline-date" min="${new Date(Date.now() + 86400000).toISOString().slice(0,10)}">
          </div>
          <div class="field">
            <label>Waktu</label>
            <input type="time" id="co-deadline-time" value="17:00">
          </div>
        </div>
        <p class="muted mt-16" style="font-size:.82rem;">⏰ Pemesanan jasa daftar pustaka maksimal H-1.</p>
      </div>
    ` : ''}
...
function onFilePick(input) {
  ...
  if (!f) return;
  const ok = ['pdf','doc','docx','jpg','jpeg','png'].includes(f.name.split('.').pop().toLowerCase());
  if (!ok) { toast('Format file tidak didukung', 'error'); input.value=''; return; }
...
function proceedToConfirm() {
  ...
  let fileName = '', deadline = '';
  if (hasService) {
    const f = $('co-file')?.files[0];
    if (!f) return toast('Upload file untuk jasa daftar pustaka', 'error');
    fileName = f.name;
    const d = $('co-deadline-date').value;
    const t = $('co-deadline-time').value;
    if (!d || !t) return toast('Tentukan permintaan selesai', 'error');
    const dt = new Date(d + 'T' + t);
    if (dt - Date.now() < 86400000 * 0.9) return toast('Pemesanan jasa maksimal H-1. Pilih waktu lain.', 'error');
    deadline = d + ' ' + t;
  }
  ...
  // Simpan file content untuk demo (nama saja karena LocalStorage terbatas)
  if (hasService && $('co-file').files[0]) {
    const reader = new FileReader();
    reader.onload = e => {
      currentOrderDraft.fileData = e.target.result;
      currentOrderDraft.fileType = $('co-file').files[0].type;
      navigate('confirm');
    };
    reader.readAsDataURL($('co-file').files[0]);
  } else {
    navigate('confirm');
  }
}
...
function renderConfirm() {
  ...
  return `
    ...
    <div class="admin-panel">
      ...
      ${d.fileName ? `<div class="pay-info-row"><span>File Jasa</span><b>📎 ${d.fileName}</b></div>` : ''}
      ${d.deadline ? `<div class="pay-info-row"><span>Permintaan Selesai</span><b>${d.deadline}</b></div>` : ''}
    </div>
    <div class="admin-panel" style="display:flex;gap:10px;align-items:center;background:var(--pink-soft);">
      <input type="checkbox" id="confirm-check" style="width:20px;height:20px;accent-color:var(--pink-deep);" onchange="document.getElementById('btn-to-pay').disabled=!this.checked">
      <label for="confirm-check" style="font-weight:600;font-size:.92rem;cursor:pointer;">Saya sudah memeriksa dan memastikan data pesanan saya sudah benar.</label>
    </div>
...
function createOrder() {
  ...
  // Hapus cart item yang sudah diorder
  const cart = DB.get('cart', {});
...
function renderPayment() {
  ...
  if (m === 'Bank Transfer') {
    ...
    detailHTML = `
      <h3>📱 Scan QRIS</h3>
      <div class="qr-wrap">
        <img id="qris-img" src="${s.qris}" alt="QRIS">
...
function renderOrderDetail(id) {
    ...
    adminActions = `
      <div class="admin-panel" style="background:var(--pink-soft);border-color:var(--pink-med);">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          ${o.paymentStatus !== 'Pembayaran Diterima' && o.paymentStatus === 'Menunggu Pembayaran' ? `<button class="btn btn-outline btn-sm" onclick="confirmPaymentReceived('${o.id}')">💰 Tandai Dibayar</button>` : ''}
        ...
        ${o.items.some(x => x.isService) ? `
          ...
          <h4 style="margin-bottom:8px;">📚 Kelola File Jasa</h4>
          <div class="mb-16">
            <label class="btn btn-outline btn-sm" style="cursor:pointer;">
              📤 Upload Hasil Jasa
              <input type="file" style="display:none;" onchange="uploadResult('${o.id}', this)">
            </label>
            ${o.resultFile ? `<span class="tag-soft" style="margin-left:8px;">✅ Hasil: ${o.resultFile}</span>` : ''}
          </div>
          ${o.fileData ? `<a class="btn btn-ghost btn-sm" download="${o.fileName}" href="${o.fileData}">📥 Download File Pembeli</a>` : ''}
        ` : ''}
      </div>
    `;
  }
  ...
  return `
    ...
    <div class="admin-panel">
      ...
      ${o.deadline ? `<div class="pay-info-row"><span>Deadline</span><b>${o.deadline}</b></div>` : ''}
      ${o.notes ? `<div class="pay-info-row"><span>Catatan</span><b>${o.notes}</b></div>` : ''}
      ${o.fileName ? `<div class="pay-info-row"><span>File Jasa</span><b>📎 ${o.fileName}</b></div>` : ''}
    </div>
...
function updateOrderStatus(id, status) {
  ...
  if (status === 'Selesai') {
    ...
    addNotification(o.userId, 'order_done', `🎉 Pesanan ${o.orderId} sudah selesai!`);
  }
  DB.set('orders', orders);
...
function attachStars() { ratingValue = 0; }
...
function renderAdmin(tab) {
  ...
  const tabs = [
    ...
    ['payments','💳 Pembayaran'],['files','📁 File Jasa'],['ratings','⭐ Rating'],
  ...
  if (tab === 'dashboard') {
    ...
    return `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:20px;">
        ${[
          ...
          <div class="admin-panel text-center" style="padding:14px;">
            ...
            <div style="font-family:Fredoka;font-size:1.25rem;color:var(--pink-dark);font-weight:700;margin:4px 0;">${v}</div>
      ...
      <div class="admin-panel">
        ...
        ${DB.get('notifications', []).filter(n => n.userId === 'admin').slice(-8).reverse().map(n => `
      ...
      <div class="admin-panel">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${orders.filter(o => o.paymentStatus !== 'Pembayaran Diterima').slice().reverse().map(o => `
                <tr>
                  <td>${o.orderId}</td>
                  <td>${o.buyerName}</td>
        ...
        ${!orders.filter(o => o.paymentStatus !== 'Pembayaran Diterima').length ? '<p class="muted text-center mt-16">Semua pembayaran sudah diverifikasi ✨</p>' : ''}
      </div>
    `;
  }

  if (tab === 'files') {
    const fileOrders = orders.filter(o => o.fileName);
    return `
      <div class="admin-panel">
        ...
        ${fileOrders.length ? `
          <div class="table-wrap">
            <table>
              <thead><tr><th>Order ID</th><th>Pembeli</th><th>File</th><th>Deadline</th><th>Hasil</th><th></th></tr></thead>
              <tbody>
                ${fileOrders.map(o => `
                  <tr>
                    <td>${o.orderId}</td>
                    <td>${o.buyerName}</td>
                    <td>${o.fileData ? `<a class="btn btn-ghost btn-sm" download="${o.fileName}" href="${o.fileData}">📥 ${o.fileName}</a>` : '📎 ' + o.fileName}</td>
                    <td>${o.deadline || '-'}</td>
                    <td>${o.resultFile ? `<span class="tag-soft">✅ ${o.resultFile}</span>` : '<span class="muted">Belum</span>'}</td>
                    <td><button class="btn btn-outline btn-sm" onclick="navigate('order-detail',{detailId:'${o.id}'})">Kelola</button></td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
          </div>
        ` : '<p class="muted text-center">Belum ada file yang di-upload.</p>'}
      </div>
    `;
  }
    ...
    <div class="modal" id="prod-modal">
      <div class="modal-box">
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>Kelipatan Min</label><input id="p-step" type="number" value="1"></div>
...
function doRegister() {
  ...
  if (!name || !email || !pass) return toast('Lengkapi semua data', 'error');
  const users = DB.get('users', []);
  ...
  $('footer').innerHTML = `
    <div class="footer-inner">
      <div>
        <h4>💗 ${s.storeName}</h4>
        <p>${s.tagline}</p>
        <p style="margin-top:10px;">Alat tulis lengkap & jasa pembuatan daftar pustaka.</p>
      </div>
      <div>
        <h4>Menu</h4>
...
// Sinkronkan render tiap ganti halaman
